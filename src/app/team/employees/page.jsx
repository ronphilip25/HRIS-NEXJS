...
const Card = ({ title, count, link }) => {
  ...
  return (
    <button
      ...
      className={`h-[118px] w-full rounded-lg p-5 shadow-lg cursor-pointer text-left bg-white 
        ${active ? ' text-blue-600' : 'text-black hover:text-blue-600'}`}
    >
      <div className="flex font-sans text-lg font-medium">{title}</div>
      <div className="flex items-center justify-between">
        <h3 className="font-sans text-4xl font-bold text-inherit">{count}</h3>
        <div className="mb-4">
          <svg
